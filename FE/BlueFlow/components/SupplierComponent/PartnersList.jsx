...
function PartnersList({ partnersItem = [], onMessageClick }) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-11/12 ml-4 mb-2 text-xs text-slate-600">
        Showing {start + 1}-{Math.min(start + pageSize, total)} of {total} (Page {page}/{Math.max(1, Math.ceil(total / pageSize))})
      </div>
